...
import { getAdminTests, updateAdminTestStatus, AdminTest, AdminTestRaw } from "@/services/adminApi";
...
const AdminTestsPage = () => {
  ...
  const loadTests = async () => {
    ...
    try {
      ...
      const rawTests = res.data.data as AdminTestRaw[];
      const mapped = rawTests.map((test) => ({
        id: test.id,
        title: test.title,
        description: test.description,
        questions: test.questions_count,
        duration: test.duration,
        completions: test.submissions,
        active: test.active,
      ...
      setFetchError("Unable to load tests from the server.");
    } finally {
      setLoading(false);
    }
  };
  ...
  const toggleActive = async (id: number) => {
    ...
    const nextStatus = test.active ? "inactive" : "active";

    try {
      await updateAdminTestStatus(id, nextStatus);
      setTests((prev) =>
