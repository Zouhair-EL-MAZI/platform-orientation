...
const TestModal = ({ test, onClose, onSave, loading }: {
  ...
  const [category, setCategory] = useState(test?.category?.toLowerCase() ?? "aptitude");
  ...
  const handleSave = () => {
    ...
    onSave({ title, description, category, duration: Number(duration) || 15 });
  };
      ...
      <div className="rounded-2xl p-6 w-full max-w-md" style={{ background: "var(--ms-bg-card)", border: "1px solid var(--ms-border-subtle)", backdropFilter: "blur(12px)" }} onClick={(e) => e.stopPropagation()}>
        ...
        <div className="space-y-4">
          <div>
            ...
            <input type="text" value={title} onChange={(e) => setTitle(e.target.value)} className="w-full mt-1 px-3 py-2 rounded-lg bg-transparent text-sm outline-none" style={{ border: "1px solid var(--ms-border-subtle)", color: "inherit" }} onFocus={(e) => (e.currentTarget.style.borderColor = "var(--ms-border-active)")} onBlur={(e) => (e.currentTarget.style.borderColor = "var(--ms-border-subtle)")} disabled={loading} />
          </div>
          <div>
            ...
            <textarea value={description} onChange={(e) => setDescription(e.target.value)} rows={3} className="w-full mt-1 px-3 py-2 rounded-lg bg-transparent text-sm outline-none resize-none" style={{ border: "1px solid var(--ms-border-subtle)", color: "inherit" }} onFocus={(e) => (e.currentTarget.style.borderColor = "var(--ms-border-active)")} onBlur={(e) => (e.currentTarget.style.borderColor = "var(--ms-border-subtle)")} disabled={loading} />
          </div>
          <div className="grid grid-cols-2 gap-3">
            <div>
              ...
              <select value={category} onChange={(e) => setCategory(e.target.value)} className="w-full mt-1 px-3 py-2 rounded-lg bg-transparent text-sm outline-none cursor-pointer" style={{ border: "1px solid var(--ms-border-subtle)", color: "inherit" }} onFocus={(e) => (e.currentTarget.style.borderColor = "var(--ms-border-active)")} onBlur={(e) => (e.currentTarget.style.borderColor = "var(--ms-border-subtle)")} disabled={loading}>
                {CAT_VALUES.map((c) => <option key={c.value} value={c.value}>{t(c.labelKey)}</option>)}
              </select>
            </div>
            <div>
              ...
              <input type="number" min={1} value={duration} onChange={(e) => setDuration(e.target.value)} className="w-full mt-1 px-3 py-2 rounded-lg bg-transparent text-sm outline-none" style={{ border: "1px solid var(--ms-border-subtle)", color: "inherit" }} onFocus={(e) => (e.currentTarget.style.borderColor = "var(--ms-border-active)")} onBlur={(e) => (e.currentTarget.style.borderColor = "var(--ms-border-subtle)")} disabled={loading} />
            </div>
          </div>
        </div>
    ...
    <div className="rounded-xl p-4 space-y-3" style={{ background: "var(--ms-bg-layer3)", border: "1px solid var(--ms-border-subtle)" }}>
      {!editing ? (
        ...
        <div className="space-y-3">
          ...
          <select value={type} onChange={(e) => setType(e.target.value)} className="w-full px-3 py-2 rounded-lg bg-transparent text-sm outline-none cursor-pointer" style={{ border: "1px solid var(--ms-border-subtle)", color: "inherit" }}>
            <option value="single_choice">{t("admin.tests.singleChoice")}</option>
            <option value="scale">{t("admin.tests.scale")}</option>
            <option value="text">{t("admin.tests.text")}</option>
          </select>
          {type === "single_choice" && (
            <div className="space-y-1.5">
              {options.map((o, i) => (
                <input key={i} value={o} onChange={(e) => { const n = [...options]; n[i] = e.target.value; setOptions(n); }} placeholder={`${t("admin.tests.option")} ${i + 1}`} className="w-full px-3 py-1.5 rounded-lg bg-transparent text-sm outline-none" style={{ border: "1px solid var(--ms-border-subtle)", color: "inherit" }} />
              ))}
            </div>
          )}
...
const QuestionsPanel = ({ test, onClose }: { test: AdminTest; onClose: () => void }) => {
  ...
  const addQuestion = async () => {
    ...
    setSaveLoading(true);
    try {
      const opts = newType === "single_choice" ? newOptions.filter((o) => o.trim()) : undefined;
      const res = await createAdminQuestion(test.id, { question: newQ, type: newType, options: opts });
  ...
  return (
    <div className="fixed inset-0 bg-transparent backdrop-blur-md flex items-center justify-center z-50 p-4 mb-50" onClick={onClose}>
      <div className="rounded-2xl w-full max-w-2xl max-h-[70vh] flex flex-col" style={{ background: "var(--ms-bg-card)", border: "1px solid var(--ms-border-subtle)", backdropFilter: "blur(12px)" }} onClick={(e) => e.stopPropagation()}>
        {/* Header */}
        <div className="flex items-center justify-between p-5 border-b" style={{ borderColor: "var(--ms-border-subtle)" }}>
          ...
          <button onClick={onClose} className="p-1.5 rounded-lg hover:bg-accent" style={{ color: "hsl(var(--muted-foreground))" }}><X size={16} /></button>
        </div>
        ...
        <div className="flex-1 min-h-0 overflow-y-auto p-5 space-y-3">
          ...
          {adding ? (
            <div className="rounded-xl p-4 space-y-3" style={{ background: "var(--ms-bg-layer3)", border: "1px solid var(--ms-border-glow)" }}>
              ...
              <select value={newType} onChange={(e) => setNewType(e.target.value)} className="w-full px-3 py-2 rounded-lg bg-transparent text-sm outline-none cursor-pointer" style={{ border: "1px solid var(--ms-border-subtle)", color: "inherit" }}>
                <option value="single_choice">{t("admin.tests.singleChoice")}</option>
                <option value="scale">{t("admin.tests.scale")}</option>
                <option value="text">{t("admin.tests.text")}</option>
              </select>
              {newType === "single_choice" && (
                <div className="space-y-1.5">
                  {newOptions.map((o, i) => (
                    <input key={i} value={o} onChange={(e) => { const n = [...newOptions]; n[i] = e.target.value; setNewOptions(n); }} placeholder={`${t("admin.tests.option")} ${i + 1}`} className="w-full px-3 py-1.5 rounded-lg bg-transparent text-sm outline-none" style={{ border: "1px solid var(--ms-border-subtle)", color: "inherit" }} />
                  ))}
                </div>
              )}
              <div className="flex gap-2">
  ...
  const loadTests = async () => {
    ...
    try {
      ...
      setTests(raw.map((t) => ({
        ...
        questions: t.questions_count, duration: t.duration,
        completions: t.submissions, active: t.active, category: t.category,
      })));
    } catch (e) { console.error(e); } finally { setLoading(false); }
  };
    ...
    <div className="max-w-7xl mx-auto space-y-6 animate-fade-in">
      ...
      <div className="rounded-2xl p-4 flex flex-wrap gap-3 items-center" style={{ background: "var(--ms-bg-card)", border: "1px solid var(--ms-border-subtle)", backdropFilter: "blur(12px)" }}>
        <div className="relative flex items-center gap-2 flex-1 min-w-[200px] max-w-sm rounded-xl h-9 px-3" style={{ background: "var(--ms-bg-layer2)", border: "1px solid var(--ms-border-subtle)" }}>
          ...
          <input type="text" placeholder={t("admin.tests.searchPlaceholder")} value={search} onChange={(e) => setSearch(e.target.value)} className="flex-1 bg-transparent text-sm outline-none placeholder:text-muted-foreground" />
        </div>
        <div className="flex items-center gap-1">
          {CATEGORIES.map((c) => (
            <button key={c.value} onClick={() => setActiveCategory(c.value)} className="px-3 py-1.5 rounded-lg text-xs font-bold transition-all duration-200"
          ...
          {filtered.map((test) => (
            <div key={test.id} className="rounded-2xl p-5 relative overflow-hidden group transition-all duration-200"
              ...
              <div className="flex items-center gap-4 text-xs mb-4" style={{ color: "hsl(var(--muted-foreground))" }}>
                ...
                <span className="flex items-center gap-1"><Users size={11} style={{ color: "var(--ms-accent-cyan)" }} />{test.completions}</span>
              </div>
